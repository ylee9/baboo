...
class TwoComponentModel(KalmanFilterTimeVarying):
    ...
    def __init__(
        ...
        super(TwoComponentModel, self).__init__(
            transition, design, Q, measurement_cov, B, solve
        )
        if times is None:
    ...
    def transformed_params(self):
        A, B, C, D = (
            self.params["relax_ratio"],
            (self.params["reduced_relax"]) ** -1,
            self.params["lag"],
            self.params["omegac_dot"],
        )
        taus = (1 + A) * B
        tauc = (1 + A) / A * B
        qc = self.params["Qc"]
    ...
    def _check_params(self, params):
        if list(self.params.keys()) != list(params.keys()):
            raise ValueError(
                f"Incorrect set of parameters supplied. List of available parameters is {self.keylist}"
            )
    ...
    def loglike(self, params, loglikelihood_burn=1, return_states=False):
        ...
        try:
            ...
            if isinstance(params, dict):
                ...
                omgs_0 = omgc_0 - lag
            return self.ll_on_data(
                self.data,
                params,
                x0=np.array([omgc_0, omgs_0]),
    ...
    def update_Q(self, tauc, taus, Qc, Qs, dts):
        ...
        Q[1, 0, :] = (
            (Qs * taus ** 2 + Qc * tauc ** 2) * dts
            + (Qc * tauc * taus - tauc ** 2 * Qc + Qs * tauc * taus - Qs * taus ** 2)
            * tau
            * (1 - expvals)
    ...
    def update_torque(self, tauc, taus, Nc, Ns, dts):
        ...
        torques[0, :] = omg_dot * dts + tau ** 2 * (
            tauc ** -1 * (Nc - Ns) * (1 - expvals)
        )
        torques[1, :] = omg_dot * dts + tau ** 2 * (
            taus ** -1 * (Ns - Nc) * (1 - expvals)
        )
        self.B = torques
...
class OneComponentModel(KalmanFilterTimeVaryingOneState):
    def __init__(
        ...
        super(OneComponentModel, self).__init__(
            transition, design, Q, measurement_cov, B
        )
        if times is None:
            raise ValueError("must specify variable dt")
        self.times = times
...
class SecondSpindownModel(KalmanFilterTimeVarying):
    ...
    def __init__(
        ...
        super(SecondSpindownModel, self).__init__(
            transition, design, Q, measurement_cov, B, solve
        )
        if times is None:
            raise ValueError("must specify variable dt")
...
class SecondSpindownModelGeneralNoise(KalmanFilterTimeVarying):
    ...
    def __init__(
        ...
        super(SecondSpindownModelGeneralNoise, self).__init__(
            transition, design, Q, measurement_cov, B, solve
        )
        if times is None:
            raise ValueError("must specify variable dt")
    ...
    def update_transition(self, beta, delta, sigma, dts):
        ...
        transition[0, 2, :] = (
            beta * sigma
            + np.exp(-dts * sigma) * (delta - beta * sigma)
            - delta * (1 - dts * sigma)
        ) / sigma ** 2
...
class MeanRevertingModel(KalmanFilterTimeVarying):
    ...
    def __init__(
        ...
        super(MeanRevertingModel, self).__init__(
            None, design, None, measurement_cov, None, solve
        )
        if times is None:
            raise ValueError("must specify variable dt")
        self.times = times
    ...
    def update_parameters(self, params):
        ...
        self.update_Q(
            params["sigma_v"], params["sigma_a"], params["gamma_v"], params["gamma_a"]
        )
        self.update_torque(
            params["gamma_v"], params["gamma_a"], params["N"], params["abar"]
        )
    ...
    def update_torque(self, gamma_v, gamma_a, N, abar):
        ...
        torque[0, :] = N * (1 - exp_gamma_v) + (
            abar
            * ((1 - exp_gamma_v) * gamma_a - (1 - exp_gamma_a) * gamma_v)
            / ((gamma_a - gamma_v) * gamma_v)
        )
    ...
    def loglike(self, params, loglikelihood_burn=1, return_states=False):
        ...
        return self.ll_on_data(
            self.data,
            params,
            x0=np.array([fdot_start, fddot_start]),
            P0=np.eye(self.nstates) * np.max(self.R[:, :, 0]) * 1e1,
            burn=loglikelihood_burn,
            return_states=return_states,
        )
...
class MeanRevertingModel2(KalmanFilterTimeVarying):
    ...
    def __init__(
        ...
        # transition, Q, and B matrices are set to None for
        # now. They will be specified when parameters are given.
        super(MeanRevertingModel2, self).__init__(
            None, design, None, measurement_cov, None, solve
        )
        if times is None:
            raise ValueError("must specify variable dt")
    ...
    def update_Q(self, sigma_v, sigma_a, gamma_v, gamma_a):
        ...
        sigv2 = sigma_v ** 2
        Q[0, 0, :] = (
            -(3 + exp_gamma_a ** 2 - 4 * exp_gamma_a - 2 * self.dts * gamma_a)
            * siga2
            / (2 * gamma_a ** 3)
            + self.dts * sigv2
        )
        Q[1, 0, :] = (
            (1 - 2 * exp_gamma_a + exp_gamma_a ** 2) * siga2 / (2 * gamma_a ** 2)
        )
        Q[0, 1, :] = Q[1, 0, :]
        Q[1, 1, :] = (1 - exp_gamma_a ** 2) * siga2 / (2 * gamma_a)
        self.Q = Q
